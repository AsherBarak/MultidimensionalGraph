...
var data1:DataElement[]=[{letter:"A",frequency:0.2},{letter:"B",frequency:0.25}];

	//	d3.tsv("data.tsv", type, function(error, data) {
	//		if (error) throw error;

			x.domain(data1.map(function(d) { return d.letter; }));
			y.domain([0, d3.max<DataElement>(data1, function(d) { return d.frequency; })]);
